...
sub _set_imager {
	...
	$self->{imager} = Imager->new unless defined $self->{imager};

	$self->{imager}->read(fh => $self->{fh});
...
sub _set_width {
	...
	$self->{width} = $self->{imager}->getwidth;
}
...
sub _set_height {
	...
	$self->{height} = $self->{imager}->getheight;
}
...
sub _create_thumbnail {
  ...
  my ($o_width, $o_height) = ($im->getwidth,$im->getheight);
  ...
  # Create thumbnail
  if( defined $im ){
    $im->scale( xpixels => $t_width, ypixels => $t_height, type => 'nonprop' );
#    $im->Set( quality => $quality );

	my(@options) = (file => $output, type => $format);

	if ( ($format eq 'jpeg') && defined($quality) )
	{
		push @options, 'jpegquality', $quality;
	}

#    $im->Set( density => $density );

	my($x, $y) = split(/x/, $density);

	if (defined($x) && ($x > 0) && defined($y) && ($y > 0) )
	{
	    $im->scale( xpixels => $x, ypixels => $y, type => 'nonprop' );
	}
...
#    $im->Set( quality => $quality );
	...
	$im->write(file => $output, type => $format);

	$self->final_width($im->getwidth);
	$self->final_height($im->getheight);
  }
}
...
See also L<jQuery::File::Upload>, from which this module was forked. The latter module requires L<Image::Magic>,
whereas this module requires L<Imager>.
...
By default, jQuery::File::Upload::Imager also assumes that you're meaning to have the uploaded file uploaded to the same server
that the script is running on. jQuery::File::Upload::Imager also has the ability to SCP files to remote servers, but this is not the default.

=item CGI environment

By default, jQuery::File::Upload::Imager assumes that the script is running in a regular CGI-type environment. However, jQuery::File::Upload::Imager
...
jQuery::File::Upload::Imager makes integrating server-side with the L<jQuery File Upload|https://github.com/blueimp/jQuery-File-Upload/> plugin simple.
...
see below for everything you can do with jQuery::File::Upload::Imager
...
When using jQuery::File::Upload::Imager under normal CGI, it should have
...
that jQuery::File::Upload::Imager is running on, it's best to just manually set this.
...
is L</upload_dir>. If you change this, that will make thumbnails
have a different base url than L</upload_url_base>. Make
sure to change L</thumbnail_url_base> to match this accordingly.
If you would like images and thumbnails to have the same name but just be in
different directories, make sure you set L</thumbnail_prefix>
...
at a different domain, then just set L</upload_url_base>
...
L</upload_url_base> is.
If your location for thumbnail images is not relative, i.e. it is located
at a different domain, then just set L</thumbnail_url_base>
to get the url_base you want. There should not be
a slash at the end.
...
If set to 1, this will make L</relative_url_path> and
L</thumbnail_relative_url_path> be relative to
the host of the script url. For example:

  http://www.mydomain.com/folder/upload.cgi

With a L</relative_url_path> '/files' would yield:

  http://www.mydomain.com/files

Whereas by default L</relative_url_path> and
L</thumbnail_relative_url_path> are
relative to the folder the upload script is running in.

If you use this option, make sure to set L</upload_dir>
(and/or L</thumbnail_upload_dir> if necessary)
since jQuery::File::Upload::Imager can no longer do a relative path
for saving the file.
...
This should be used mostly internally by jQuery::File::Upload::Imager
...
This is used to decide whether to actually delete the files when jQuery::File::Upload::Imager
receives a DELETE request. The default is to delete, however this could be useful
if you wanted to maybe just mark the field as deleted in your database (using L</pre_delete>)
...
just let jQuery::File::Upload::Imager generate the filename. Please note
that if you choose your own filename, you do have to manually set
L</thumbnail_filename>
...
You can also set this manually if you would like, or jQuery::File::Upload::Imager
...
L</handle_request>. However, unless you're sure
that you are going to give the file a unique name, you should
just let jQuery::File::Upload::Imager generate the filename.
...
This can be used to set whether jQuery::File::Upload::Imager shows the user the name
...
filename will look better than the unique one that jQuery::File::Upload::Imager generates
for you.
...
If this is set to true, jQuery::File::Upload::Imager will use
...
between two different users. It is best to let jQuery::File::Upload::Imager
...
This can be used to set the url of the script that jQuery::File::Upload::Imager is
running under. jQuery::File::Upload::Imager then uses this value to generate
other parts of the output. jQuery::File::Upload::Imager in most cases is able
...
can access any outside data you need by calling L</data> on
the jQuery::File::Upload::Imager object that you are passed. However, keep in mind
that if you are using L<Catalyst>, you will have access to the context
object via the jQuery::File::Upload::Imager object that is passed in, and this
...
call L</print_response> as this prints out
the header and the JSON for you (or alternatively call L</handle_response>
and pass it a 1 so that it will call L</print_response> for you.
...
Should be called after L</handle_request>.
Prints out header and JSON back to browser. Called for
convenience by L</handle_request> if
L</handle_request> is passed a 1.
...
If passed a 1, will also call L</print_response>
after it's finished.
...
This should be used in conjuction with L</pre_get>
...
method is called, you will need to call L</print_response>
or L</handle_request> with a 1 to print out the JSON.
...
thumbnail_filename - filename for thumbnail. jQuery::File::Upload::Imager will generate the thumbnail_filename based
on the filename and other factors (such as L</upload_url_base>) if you don't set this.
...
delete the file. It's better to just let jQuery::File::Upload::Imager generate this and use L</delete_params>
if you want to set your own parameters for the delete url.

=item

delete_params - The format of this is just like L</delete_params>. It takes [key,value] pairs.
Any values here will be added in addition to any global L</delete_params> that you set.
...
Note that jQuery::File::Upload::Imager will generate urls and such
based upon things given here (like filename) and other
options such as L</upload_url_base>.

=head2 PRE/POST REQUEST METHODS

N.B. The following functions are all passed a jQuery::File::Upload::Imager object. And they
can be passed into L</new> as options.

Also, note that since all of these user-defined methods are passed the jQuery::File::Upload::Imager object,
if you are using L<Catalyst> you can just call the L</ctx> method to get anything
stored via your context object. For L<Catalyst> users, this makes this a practical (and possibly better)
alternative to the provided L</data> method.
...
database. Also, you can use this along with L</delete_params>
...
L</should_delete> is set to false.
...
You can use the jQuery::File::Upload::Imager object that
...
load and using L</generate_output> to
